...
{
    vector<float> aryGeneSerial;    // 染色體序列.
    int geneDimension;              // 序列維度.
    float fitness;                  // 適性值.
    int toNextGeneration;           // 下一代的遺傳個數(依群體適性值比例計算)
    ...
    void initGeneSerialArray(int dimensionOfChromosome)
    {
        if ( dimensionOfChromosome > 0 )
        {
            ...
            aryGeneSerial.resize(geneDimension);
        }
    }
    ...
    void copyFromInput(const stParticule &p)
    {
        ...
        {
            aryGeneSerial[i] = p.aryGeneSerial[i];
        }
        fitness = p.fitness;
        toNextGeneration = p.toNextGeneration;
    }

    void randomlyAssignGeneSerialArray()
    {
        for ( int i=0 ; i < geneDimension ; i++ )
        {
            // -1 ~ 1
            float random = (double)rand()*2 / RAND_MAX + (-1);
            aryGeneSerial[i] = random;
        }
    }
